 ...
 *                6.SUB/SUBC分别用于在接口及抽象类、实类中的函数内获得继承子类指针cthis；
 *                7.FS配置实类中的参数，不限于函数，实类参数配置尽可能通过FS配置，而不是init进行传参配置；
 *                8.CN/CF实例化或释放实类对象；
 *                9.由于CN/CF/CCC操作后没有返回值，可以通过OPRS查看前一步操作结果；
...
// 在构造函数中对类的参数（不止是函数，其他变量也行）进行配置 function,setting-----
...
// 由于CN/CF/CCC调用后没有返回值，通过以下宏查看操作结果operation result，self/OOPC_NULL
